...
public final class NearbyMapActivity extends AppCompatActivity {
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    // Inject the map fragment into the container
    SupportMapFragment smf = SupportMapFragment.newInstance();
    getSupportFragmentManager().beginTransaction()
      .replace(R.id.mapContainer, smf)
      .commit();

    // Make sure we configure the map view's Z-order so our overlays are visible
    getSupportFragmentManager().executePendingTransactions();
    ensureMapBelowOverlays(smf);
    ...
    // Wire up overlay controls (must exist in activity_nearby_map.xml)
    Button btnMy = findViewById(R.id.btnMyLocation);
    Button btnRefresh = findViewById(R.id.btnRefresh);
    View cardControls = findViewById(R.id.cardControls);
    View btnClose = findViewById(R.id.btnClose); // make sure this ID exists in your XML

    // Force overlays above the map surface
  ...
  /**
   * Ensures the Google Map view (GLSurface/Texture) does NOT sit on top of overlays.
   * Works for SupportMapFragment embedded in a FrameLayout.
   */
  private void ensureMapBelowOverlays(SupportMapFragment smf) {
    View mapView = smf.getView();
    if (mapView == null) return;

    // Try common map child types and keep them below app overlays
    ViewParent parent = mapView.getParent();
    if (mapView instanceof SurfaceView) {
      // SurfaceView can punch through; force it below
      ((SurfaceView) mapView).setZOrderOnTop(false);
    } else if (mapView instanceof TextureView) {
      // TextureView respects Z; ensure it doesn't occlude overlays
      ((TextureView) mapView).setOpaque(false);
    }

    // Keep the map's own Z/elevation low
    ViewCompat.setTranslationZ(mapView, 0f);
    ViewCompat.setElevation(mapView, 0f);
    mapView.setZ(0f);
  }

  /** Bring overlay views above the map surface reliably. */
  private void bringOverlaysToFront(View... overlays) {
    float z = 1000f; // large enough to be above GLSurface/Texture
    for (View v : overlays) {
      if (v == null) continue;
      v.bringToFront();
      ViewCompat.setTranslationZ(v, z);
      ViewCompat.setElevation(v, z);
      v.setZ(z);
      v.requestLayout();
      v.invalidate();
    }
    ...
    new Thread(() -> {
      try {
        ...
        runOnUiThread(() -> {
          ...
          // Reassert overlays on top after map draws (defensive)
          bringOverlaysToFront(findViewById(R.id.cardControls),
